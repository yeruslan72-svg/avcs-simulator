...
def calculate_features(signal_data):
    if len(signal_data) == 0:
        return {'rms': 0, 'pkpk': 0, 'crest': 0, 'variance': 0,
                'centroid': 0, 'dominant_freqs': [0, 0], 'kurtosis': 0}
    ...
    try:
        ...
        kurtosis_val = (np.mean(magnitude**4) /
                        (np.mean(magnitude**2)**2) - 3 if np.mean(magnitude**2) > 0 else 0)
    except Exception:
        centroid = 0
        dominant_freqs = [0, 0]
        kurtosis_val = 0
...
def generate_linkedin_post(fault_type, severity, prevented_hours,
                           potential_savings, roi, fig):
    buf = io.BytesIO()
    try:
...
with col1:
    ...
    run_simulation = st.button("▶️ Start Simulation", type="primary", key="run")
...
if run_simulation:
    ...
    if fault_type == "Normal Operation":
        ...
        harmonic_strength = 0.4 * severity
        harmonic_2x = harmonic_strength * np.sin(2 * np.pi * 100 * t)
...
st.markdown("---")
st.markdown("**Operational Excellence, Delivered** | © 2025 AVCS DNA Technology Simulator v3.5")
